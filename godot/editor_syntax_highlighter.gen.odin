...
editor_syntax_highlighter__get_supported_languages :: proc "contextless" (
    ...
    @(static) __ptr: __bindgen_gde.MethodBindPtr
    if __ptr == nil {
        _gde_name := new_string_name_cstring("_get_supported_languages", true)
...
editor_syntax_highlighter__create :: proc "contextless" (
    ...
    @(static) __ptr: __bindgen_gde.MethodBindPtr
    if __ptr == nil {
        _gde_name := new_string_name_cstring("_create", true)
        __ptr = __bindgen_gde.classdb_get_method_bind(&__class_name, &_gde_name, 3789807118)
    }
    self := self
    args := []__bindgen_gde.TypePtr {
    }

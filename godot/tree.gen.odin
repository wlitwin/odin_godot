...
new_tree :: proc "contextless" () -> Tree {
    return cast(Tree)__bindgen_gde.classdb_construct_object(tree_name_ref())
}
...
tree_get_custom_drawing_canvas_item :: proc "contextless" (
    self: Tree,
) -> (ret: Rid) {
    @(static) __ptr: __bindgen_gde.MethodBindPtr
    if __ptr == nil {
        _gde_name := new_string_name_cstring("get_custom_drawing_canvas_item", true)

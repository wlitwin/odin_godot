...
// Signal/arg names from `//gd:signal` markers are spliced into generated Odin code (proc
// names, params, globals), so anything else must be rejected at parse time.
...
scan_markers :: proc(src: string, s: ^Script, pkg_line: int) {
	...
	for line in strings.split_lines_iterator(&it) {
		...
		if rest, ok := marker_arg(body, "extends"); ok {
			...
			s.marked = true
			parse_signal_marker(loc, strings.trim_space(rest), s)
		} else {
			...
			error_at(loc, "unknown //gd: marker %q (expected one of: extends/class/tool/icon/signal)", body)
		}
	}
}
...
parse_signal_marker :: proc(loc: Loc, decl: string, s: ^Script) {
	open := strings.index(decl, "(")
	if open < 0 {
		error_at(loc, "malformed //gd:signal (missing '('): %q", decl)
		return
	}
	...
	// The name is spliced into generated identifiers (`<snake>_emit_<name>`, `_sig_<name>_name`)
	// and registered with the engine — anything but a plain identifier breaks the gen file.
	if !is_odin_ident(name) {
		error_at(loc, "//gd:signal name %q is not a valid identifier", name)
		return
	}
	// Two signals with one name: the engine keeps only one registration and the gen file
	// would define `_sig_<name>_name` (and the emit helper) twice.
	for prev in s.signals {
		if prev.name == name {
			error_at(loc, "duplicate //gd:signal %q (already declared at %s:%d)", name, s.path, prev.line)
			return
		}
	}
	sig := Signal_Info {
	...
	if params != "" {
		for part in strings.split(params, ",") {
			...
			if colon < 0 {
				error_at(loc, "signal %q arg %q missing `: type`", name, p)
				continue
			}
			aname := strings.trim_space(p[:colon])
			// Arg names become emit-helper parameters and registered cstring literals.
			if !is_odin_ident(aname) {
				error_at(loc, "signal %q arg name %q is not a valid identifier", name, aname)
				continue
			}
			atype := normalize_godot_qualifier(strings.trim_space(p[colon + 1:]), s.godot_alias)
			vi, ok := map_variant(atype)
			if !ok {
				error_at(loc, "signal %q arg %q: unsupported type %q", name, aname, atype)
				continue
			}

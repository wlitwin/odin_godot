...
emit_backup :: proc(b: ^strings.Builder, s: ^Script) {
	...
	for bk in s.backups {
		...
		case .Pod:
			fmt.sbprintf(
				b,
				"#assert(%s, \"%s.%s: gd:\\\"backup\\\" fields must be POD (no strings, slices, maps, or pointers)\")\n",
				pod(field_type_expr(cls, bk.path)),
				cls,
				bk.field,
			)
		case .Map:
			fmt.sbprintf(
				b,
				"#assert(%s, \"%s.%s: gd:\\\"backup\\\" map key must be POD\")\n",
				pod(bk.key),
				cls,
				bk.field,
			)
			fmt.sbprintf(
				b,
				"#assert(%s, \"%s.%s: gd:\\\"backup\\\" map value must be POD\")\n",
				pod(bk.elem),
				cls,
				bk.field,
			)
		case .Dyn:
			fmt.sbprintf(
				b,
				"#assert(%s, \"%s.%s: gd:\\\"backup\\\" element must be POD\")\n",
				pod(bk.elem),
				cls,
				bk.field,
			)
...
script_needs :: proc(s: ^Script) -> (n: Gen_Needs) {
	...
	event_boot := len(s.net_events) > 0 || len(s.facts) > 0
	...
	n.ksess =
		len(s.entities) > 0 ||
		has_lane_wiring ||
		len(s.std_forwards) > 0 ||
		s.step_boot ||
		len(s.event_halves) > 0 ||
		has_succ ||
		s.profile_type != "" ||
		len(s.messages) > 0 ||
		coop_commands ||
...
generate_all :: proc(pkg: string, sections: []Gen_Section) -> string {
	...
	fmt.sbprintf(
		&b,
		"// One artifact per scripts dir: %d script section(s), banner-marked below.\n",
		len(sections),
	)
...
// Emit the callback-free half of every generated action descriptor. Immediate
// and scheduled execution tables both embed this exact shape; only their
// callback-bearing outer structs differ.
emit_action_metadata :: proc(b: ^strings.Builder, s: ^Script) {
	w :: strings.write_string
	...
	for c in s.commands {
		if len(c.args) > 0 {
			fmt.sbprintf(
				b,
				"@(private = \"file\")\n_%s_action_%s_args := [?]knet.Action_Argument_Desc {{\n",
				snake,
				c.name,
			)
			for arg in c.args {
				fmt.sbprintf(
					b,
					"\t{{name = %q, type_name = %q, wire_kind = %q}},\n",
					arg.name,
					arg.type_text,
					arg.wire,
				)
		...
		if len(c.outcomes) > 0 {
			fmt.sbprintf(
				b,
				"@(private = \"file\")\n_%s_action_%s_outcomes := [?]knet.Action_Outcome_Value_Desc {{\n",
				snake,
				c.name,
			)
...
emit_action_desc :: proc(
	...
	fmt.sbprintf(
		b,
		"action = {{name = %q, id = %s_CMD_%s, model = .%s, policy = %s_POLICY_%s",
		c.name,
		upper,
	...
	if len(c.args) > 0 {
		fmt.sbprintf(b, ", arguments = _%s_action_%s_args[:]", snake, c.name)
	}
	if len(c.outcomes) > 0 {
		fmt.sbprintf(b, ", outcomes = _%s_action_%s_outcomes[:]", snake, c.name)
	}
	if c.then_proc != "" {
		fmt.sbprintf(
			b,
...
emit_script :: proc(b: ^strings.Builder, s: ^Script) {
	...
	// The recursive canonical ABI was computed before emission. Pin the consumer
	// compiler to those exact widths/alignments: native and wasm may share a
	// fingerprint only when their concrete raw layouts share the bytes too.
	if s.tick.input_type != "" && s.tick.input_abi_size > 0 {
		fmt.sbprintf(
			b,
			"#assert(size_of(%s) == %d && align_of(%s) == %d, \"%s: target layout differs from NET_SCHEMA_CANONICAL; use fixed-width, padding-free fields\")\n",
			s.tick.input_type,
			s.tick.input_abi_size,
			s.tick.input_type,
			s.tick.input_abi_align,
			s.tick.input_type,
		)
	}
	for ic in s.input_classes {
		if ic.abi_size <= 0 {continue}
		fmt.sbprintf(
			b,
			"#assert(size_of(%s) == %d && align_of(%s) == %d, \"%s: target layout differs from NET_SCHEMA_CANONICAL; use fixed-width, padding-free fields\")\n",
			ic.type_name,
	...
	if s.profile_type != "" && s.profile_abi_size > 0 {
		fmt.sbprintf(
			b,
			"#assert(size_of(%s) == %d && align_of(%s) == %d, \"%s: target layout differs from NET_SCHEMA_CANONICAL; use fixed-width, padding-free fields\")\n",
			s.profile_type,
	...
	if s.tick.input_abi_size > 0 ||
	   len(s.input_classes) > 0 ||
	   s.profile_abi_size > 0 ||
	   len(s.messages) > 0 {w(b, "\n")}
	...
	for lc in s.lifecycles {
		...
		if lc.keyword == "process" || lc.keyword == "physics_process" {
			...
			if lc.keyword == "ready" && s.profile_type != "" {
				...
				fmt.sbprintf(
					b,
					"\tksess.session_profile_install(&(cast(^%s)self_raw).%s, %s) // gd:\"profile\"\n",
					cls,
					s.profile_ses,
					s.profile_type,
				)
...
emit_signal_helpers :: proc(b: ^strings.Builder, s: ^Script) {
	...
	for sig in s.signals {
		fmt.sbprintf(
			b,
			"// emit the `%s` signal (declared by the %s.%s field) with its payload.\n",
			sig.name,
			cls,
			sig.name,
		)
...
emit_registration :: proc(b: ^strings.Builder, s: ^Script) {
	...
	for m in s.methods {
		...
		fmt.sbprintf(
			b,
			"@(private = \"file\")\n_%s_%s_args := [?]gdext.Variant_Type{{",
			snake,
			m.gd_name,
		)
		for a, i in m.args {
	...
	if len(s.connections) > 0 {
		...
		for c in s.connections {
			...
			case c.indexed:
				fmt.sbprintf(
					b,
					"\t{{signal = \"%s\", method = \"%s\", path = \"%s\", indexed = true}},\n",
					c.signal,
					c.method,
					c.path,
				)
	...
	// kit/net replication descriptor. The recursive ABI walk already rejected
	// target-sensitive leaves and padding. These #asserts pin its expected
	// size/alignment where the field TYPE is actually known (the consumer compile).
	// `type_of(Cls{}.f)` is unevaluated: no instance is constructed.
	if len(s.replicates) > 0 {
		for r in s.replicates {
			...
			fmt.sbprintf(
				b,
				"#assert(size_of(%s) == %d && align_of(%s) == %d, \"%s.%s: target layout differs from NET_SCHEMA_CANONICAL; use fixed-width, padding-free fields\")\n",
				te,
				r.abi_size,
				te,
				r.abi_align,
		...
		for r in s.replicates {
			...
			fmt.sbprintf(
				...
				field_offset_expr(cls, r.path),
				field_type_expr(cls, r.path),
				join_path(r.path),
				flags,
				lerp,
				wire,
	...
	if len(s.commands) > 0 && !is_sim {
		...
		for c in s.commands {
			fmt.sbprintf(
				b,
				"%s_CMD_%s :: knet.Cmd_Id(0x%x)\n",
				upper,
				strings.to_upper(c.name),
				cmd_wire_id(c.name),
			)
			fmt.sbprintf(b, "%s_POLICY_%s :: %s\n", upper, strings.to_upper(c.name), c.policy_expr)
		}
		w(b, "\n")
		for c in s.commands {
			...
			if c.then_proc != "" {
				...
				if c.then_game != "" {
					fmt.sbprintf(
						b,
						"\t\tassert(env.user != nil, \"%s needs the game pointer — session_set_factory's user is what consequences receive (tests: set ctx.game_user)\")\n",
						c.then_proc,
					)
					fmt.sbprintf(
						b,
						"\t\t%s(cast(^%s)env.user, self, env.by",
						c.then_proc,
						c.then_game,
					)
				} else {
		...
		for &c in s.commands {
			w(b, "\t{")
			emit_action_desc(b, s, &c, "Immediate")
			fmt.sbprintf(b, ", invoke = _%s_cmd_%s},\n", snake, c.name)
		}
		w(b, "}\n\n")
	}
	...
	if has_edges {
		for r in s.replicates {
			...
			if r.edge_game != "" {
				fmt.sbprintf(
					b,
					"\tassert(game != nil, \"%s needs the game pointer — session_set_factory's user is what edge halves receive\")\n",
					r.edge_proc,
				)
				fmt.sbprintf(
					b,
					"\t%s(cast(^%s)game, self, (cast(^type_of(self.%s))old)^, self.%s)\n",
					r.edge_proc,
					r.edge_game,
					join_path(r.path),
					join_path(r.path),
				...
				fmt.sbprintf(
					b,
					"\t%s(self, (cast(^type_of(self.%s))old)^, self.%s)\n",
					r.edge_proc,
					join_path(r.path),
					join_path(r.path),
				)
	...
	if len(s.replicates) > 0 || len(s.commands) > 0 {
		cmds :=
			len(s.commands) > 0 && !is_sim ? fmt.tprintf(", commands = _%s_commands[:]", snake) : ""
		net_id :=
			s.net_id_type != "" ? fmt.tprintf(", net_id_offset = int(offset_of(%s, net_id))", cls) : ""
	...
	if len(s.commands) > 0 && !is_sim {
		for c in s.commands {
			...
			fmt.sbprintf(
				b,
				"// Issue `%s` — the SAME call on every peer AND on both models, zero role\n",
				c.name,
			)
			w(b, "// branches. Authority: runs the proc directly. Client: applies the generated\n")
			w(b, "// typed action policy, optionally predicts, and sends when admitted.\n")
			w(b, "// Returns a knet.Action_Outcome — .state is Applied/Predicted/Rejected,\n")
			w(
				b,
				"// .reason names a synchronous refusal, and .seq correlates the final callback.\n",
			)
			w(
				b,
				"// `knet.command_ok(r)` is the \"applied on my screen?\" check. The boot is the\n",
			)
			...
			w(b, ") -> knet.Action_Outcome {\n")
			w(b, "\tctx := &b.ses.ctx\n")
			fmt.sbprintf(
				b,
				"\tif _reason := ksess.session_action_rejection(b.ses, self.net_id, %s_POLICY_%s); _reason != .None {{return knet.action_outcome(.Rejected, _reason)}}\n",
				upper,
				strings.to_upper(c.name),
			)
			...
			if c.then_proc != "" {
				...
				if c.then_game != "" {
					fmt.sbprintf(
						b,
						"\t\t\tassert(ctx.game_user != nil, \"%s needs the game pointer — session_set_factory's user is what consequences receive (tests: set ctx.game_user)\")\n",
						c.then_proc,
					)
					fmt.sbprintf(
						b,
						"\t\t\t%s(cast(^%s)ctx.game_user, self, ctx.me",
						c.then_proc,
						c.then_game,
					)
				} else {
			...
			fmt.sbprintf(
				b,
				"\t\tknet.command_hook_local(ctx, self.net_id, %s_CMD_%s, _ok) // same cross-entity path client commands take\n",
				upper,
				strings.to_upper(c.name),
			)
			w(
				b,
				"\t\tif _ok {return knet.action_outcome(.Applied)}\n\t\treturn knet.action_outcome(.Rejected, .Predicate)\n\t}\n",
			)
			fmt.sbprintf(
				b,
				"\tknet.command_begin(ctx, self.net_id, %s_CMD_%s)\n",
				upper,
				strings.to_upper(c.name),
			)
			for a in c.args {
			...
			fmt.sbprintf(
				b,
				"\t_issue := knet.command_issue(ctx, self, &%s_command_set, %s_CMD_%s)\n",
				snake,
				upper,
				strings.to_upper(c.name),
			)
	...
	// one game-owned route per message, a `<class>_messages` proc the generated
	// net facade calls (custom shells call it once), and the send doors. NOT
	// lane-scoped — a coop game and a sim game get these identically
	...
	if len(s.messages) > 0 {
		...
		w(b, "\n")
		w(b, "// Register every @(gd_message) route on this class. The generated net facade\n")
		fmt.sbprintf(
			b,
			"// calls this; a custom shell calls `%s_messages(self, &self.ses)` once\n",
			snake,
		)
		w(b, "// after boot_attach. Routes survive *_start.\n")
		...
		for m in s.messages {
			fmt.sbprintf(
				b,
				"// Send a %s under the %q route to `to_peer` (ksess.HOST_PEER, a seated\n",
				m.payload_type,
				m.name,
			)
			...
			fmt.sbprintf(
				b,
				"\tksess.session_app_send_typed(s, %s, msg, to_peer, channel)\n}}\n\n",
				m.tag_ident,
			)
			fmt.sbprintf(
				b,
				"// Send a %s to a specific PLAYER (the seat is resolved on the authority).\n",
				m.payload_type,
			)
			fmt.sbprintf(
				b,
				"%s_send_to :: proc(s: ^ksess.Session, player: knet.Player_Id, msg: %s) {{\n",
				m.proc_name,
				m.payload_type,
			)
			fmt.sbprintf(
				b,
				"\tksess.session_app_send_typed_to(s, player, %s, msg)\n}}\n\n",
				m.tag_ident,
			)
		}
	}
	...
	if s.tick.proc_name != "" || len(s.block_ticks) > 0 {
		...
		if s.tick.proc_name != "" && s.tick.input_type != "" {
			w(
				b,
				"\tif input == nil {return} // no input drives this entity here this tick: coast\n",
			)
		}
		...
		if s.tick.then_proc != "" {
			...
			if s.tick.then_game != "" {
				fmt.sbprintf(
					b,
					"assert(ksim.lane_game(lane) != nil, \"%s needs the game pointer — lane_set_sim's user is what tick consequences receive\")\n\t\t",
					s.tick.then_proc,
				)
				fmt.sbprintf(
					b,
					"%s(cast(^%s)ksim.lane_game(lane), self, owner",
					s.tick.then_proc,
					s.tick.then_game,
				)
			} else {
		...
		if s.tick.fx_proc != "" && !s.tick.fx_mine {
			...
			if s.tick.fx_game != "" {
				fmt.sbprintf(
					b,
					"%s(cast(^%s)ksim.lane_game(lane), self",
					s.tick.fx_proc,
					s.tick.fx_game,
				)
			} else {
		...
		if s.tick.fx_proc != "" && s.tick.fx_mine {
			...
			if s.tick.fx_game != "" {
				fmt.sbprintf(
					b,
					"%s(cast(^%s)ksim.lane_game(lane), self, _mine",
					s.tick.fx_proc,
					s.tick.fx_game,
				)
			} else {
		...
		if s.tick.fx_proc != "" && s.tick.fx_mine {
			...
			if s.tick.fx_game != "" {
				fmt.sbprintf(
					b,
					"%s(cast(^%s)ksim.lane_game(lane), self, mine",
					s.tick.fx_proc,
					s.tick.fx_game,
				)
			} else {
		...
		if len(s.commands) > 0 {
			...
			for c in s.commands {
				...
				fmt.sbprintf(
					b,
					"@(private = \"file\")\n_%s_simcmd_%s :: proc(entity: rawptr, args: []u8, lane: ^ksim.Lane, by: knet.Player_Id) -> knet.Action_Reject_Reason {{\n",
					snake,
					c.name,
				)
				fmt.sbprintf(b, "\tself := cast(^%s)entity\n", cls)
				...
				w(b, "\tif r.err || len(knet.reader_remaining(&r)) != 0 {return .Malformed}\n")
				w(b, "\t_ok")
				for i in 0 ..< c.payload_count {
					if c.then_proc != "" {
						fmt.sbprintf(b, ", _p%d", i)
				...
				if c.then_proc != "" {
					...
					if c.then_game != "" {
						fmt.sbprintf(
							b,
							"\t\tassert(ksim.lane_game(lane) != nil, \"%s needs the game pointer — lane_set_sim's user is what consequences receive\")\n",
							c.then_proc,
						)
						fmt.sbprintf(
							b,
							"\t\t%s(cast(^%s)ksim.lane_game(lane), self, by",
							c.then_proc,
							c.then_game,
						)
					} else {
			...
			for &c in s.commands {
				w(b, "\t{")
				emit_action_desc(b, s, &c, "Scheduled")
				if c.apply_proc != "" {
					fmt.sbprintf(
						b,
						", exec = _%s_simcmd_%s, apply = _%s_simcmd_%s_apply},\n",
						snake,
						c.name,
						snake,
						c.name,
			...
			for c in s.commands {
				...
				fmt.sbprintf(
					b,
					"// Issue `%s` — tick-scheduled on the sim lane: the SAME call on every peer\n",
					c.name,
				)
				w(
					b,
					"// AND on both models, zero role branches. Your own entity speculates at your\n",
				)
				w(
					b,
					"// next tick; the authority executes at the stamped tick and answers with a\n",
				)
				w(
					b,
					"// verdict — a rejection unwinds the delta-lane writes and the reconcile\n",
				)
				w(b, "// scrubs the predicted ones. Returns a knet.Action_Outcome: .Predicted =\n")
				w(
					b,
					"// scheduled and in flight on my screen (even the authority's own issue runs\n",
				)
				w(
					b,
					"// at the stamped tick, never inline — .Applied is a coop-loop word); the\n",
				)
				w(
					b,
					"// verdict is state. `knet.command_ok(r)` reads it, same as any coop verb.\n",
				)
				fmt.sbprintf(b, "%s_cmd :: proc(b: ^kboot.Boot, self: ^%s", wrapper, cls)
				for a in c.args {
					fmt.sbprintf(b, ", %s: %s", a.name, a.type_text)
				...
				fmt.sbprintf(
					b,
					"\tassert(b.lane != nil, \"%s_cmd rides the sim lane — kboot.boot_lane(&boot, &lane) installs it\")\n",
					wrapper,
				)
				if len(c.args) > 0 {
					...
					fmt.sbprintf(
						b,
						"\t_issue := ksim.lane_command(b.lane, self.net_id, %s_CMD_%s, knet.writer_bytes(&_w))\n",
						upper,
						strings.to_upper(c.name),
					)
				} else {
					fmt.sbprintf(
						b,
						"\t_issue := ksim.lane_command(b.lane, self.net_id, %s_CMD_%s, nil)\n",
						upper,
						strings.to_upper(c.name),
					)
				}
				w(
					b,
					"\tif _issue.scheduled {return knet.action_outcome(.Predicted, seq = u32(_issue.seq), model = .Scheduled)}\n",
				)
				w(
					b,
					"\treturn knet.action_outcome(.Rejected, _issue.reason, model = .Scheduled)\n}\n\n",
				)
			}
		}
		...
		in_class :=
			s.tick.input_type != "" && s.tick.input_class != 0 ? fmt.tprintf(", input_class = %d", s.tick.input_class) : ""
	...
	// @(gd_step="authority") on the host alone. The generated net facade wraps
	// lane_init + boot_lane for a conventional shell; custom shells may still
	// call both directly. (A
	...
	if len(s.input_classes) > 0 ||
	   ...
	   (s.step_auth.proc_name != "" && !s.step_boot) {
		...
		for ic in s.input_classes {
			...
			if ic.validate != "" || len(ic.constraints) > 0 {
				fmt.sbprintf(
					b,
				...
				for rule in ic.constraints {
					if rule.finite && !rule.unit {
						if rule.array_len > 0 {
							fmt.sbprintf(
								b,
								"\tfor component in value.%s {{if !ksim.input_finite(component) {{return false}}}}\n",
								rule.field,
							)
						} else {
							fmt.sbprintf(
								b,
								"\tif !ksim.input_finite(value.%s) {{return false}}\n",
								rule.field,
							)
						}
					}
					if rule.unit {
						fmt.sbprintf(
							b,
							"\tif !ksim.input_unit(&value.%s) {{return false}}\n",
							rule.field,
						)
					}
					if rule.range_min != "" {
						if rule.array_len > 0 {
							fmt.sbprintf(
								b,
								"\tfor &component in value.%s {{component = clamp(component, %s, %s)}}\n",
								rule.field,
								rule.range_min,
								rule.range_max,
							)
					...
					if rule.mask != "" {
						if rule.array_len > 0 {
							fmt.sbprintf(
								b,
								"\tfor component in value.%s {{if component & ~%s(%s) != 0 {{return false}}}}\n",
								rule.field,
								rule.type_text[strings.index_byte(rule.type_text, ']') + 1:],
								rule.mask,
							)
						} else {
							fmt.sbprintf(
								b,
								"\tif value.%s & ~%s(%s) != 0 {{return false}}\n",
								rule.field,
								rule.type_text,
								rule.mask,
							)
					...
					if rule.enum_check {
						fmt.sbprintf(
							b,
							"\tif !ksim.input_enum_valid(value.%s) {{return false}}\n",
							rule.field,
						)
					}
				}
				if ic.validate != "" {
					fmt.sbprintf(b, "\treturn %s(cast(^%s)user, value)\n", ic.validate, cls)
				} else {
					w(b, "\t_ = user\n\treturn true\n")
				}
				w(b, "}\n\n")
		...
		if s.step_auth.proc_name != "" {
			fmt.sbprintf(
				...
				snake,
				s.step_auth.proc_name,
				cls,
				s.step_auth.wants_tick ? ", tick" : "",
			)
		}
		step_ref := s.step.proc_name != "" ? fmt.tprintf("_%s_lane_step", snake) : "nil"
		step_auth_ref :=
			s.step_auth.proc_name != "" ? fmt.tprintf("_%s_lane_step_auth", snake) : "nil"
		...
		if len(s.input_classes) > 0 {
			fmt.sbprintf(
				b,
				"\t#assert(%d <= ksim.MAX_INPUT_CLASSES_PER_SEAT, \"generated input classes exceed kit/sim's per-seat admission cap\")\n",
				len(s.input_classes),
			)
		}
		if len(s.input_classes) > 0 && s.input_classes[0].type_name != "" {
			fmt.sbprintf(
				b,
				"\tksim.lane_class_set_type(l, 0, typeid_of(%s))\n",
				s.input_classes[0].type_name,
			)
			if s.input_classes[0].validate != "" || len(s.input_classes[0].constraints) > 0 {
				fmt.sbprintf(
					b,
					"\tksim.lane_class_set_validate(l, 0, _%s_lane_validate_0)\n",
					snake,
				)
			}
		}
		if len(s.input_classes) > 1 {
			for ic in s.input_classes[1:] {
				sample_ref :=
					ic.sample != "" ? fmt.tprintf("_%s_lane_sample_%d", snake, ic.class_id) : "nil"
				fmt.sbprintf(
					b,
					"\tksim.lane_add_input_class(l, %d, size_of(%s), %s)\n",
					ic.class_id,
					ic.type_name,
					sample_ref,
				)
				fmt.sbprintf(
					b,
					"\tksim.lane_class_set_type(l, %d, typeid_of(%s))\n",
					ic.class_id,
					ic.type_name,
				)
				if ic.validate != "" || len(ic.constraints) > 0 {
					fmt.sbprintf(
						b,
						"\tksim.lane_class_set_validate(l, %d, _%s_lane_validate_%d)\n",
						ic.class_id,
						snake,
						ic.class_id,
					)
		...
		// ---- sim-routed @(gd_event): generated presentation doors ----------
		// The author wrote `<event>_fx`; each door here
		...
		if len(s.facts) > 0 {
			w(b, "// ---- declared sim presentation events: the announce doors ----\n")
			...
			for f in s.facts {
				fmt.sbprintf(
					b,
					"// Announce %q — call it where the sim discovers the event (the world\n",
					f.name,
				)
				...
				if f.anchor != "" {
					w(
						b,
						"// in it stay silent. Announced on a CORPSE (an anchor the game already\n",
					)
					w(
						b,
						"// despawned/untracked) it shows nowhere — not on the wire, not here, not\n",
					)
					w(
						b,
						"// later on a watcher: a fact meant to be seen is announced BEFORE the despawn.\n",
					)
				} else {
				...
				for entity_name, i in f.entity_names {
					...
					fmt.sbprintf(
						b,
						"\t_entity_id%d, _entity_ok%d := ksim.lane_entity_id(l, %s)\n",
						i,
						i,
						entity_name,
					)
					fmt.sbprintf(
						b,
						"\tif !_entity_ok%d {{return}} // a typed event ref must still be lane-tracked\n",
						i,
					)
				}
				w(b, "\tif ksim.lane_is_authority(l) {\n")
				w(b, "\t\t_fw := knet.writer_make(64, context.temp_allocator)\n")
				for _, i in f.entity_names {
					if i == f.anchor_index {continue}
					fmt.sbprintf(b, "\t\tknet.write_net_id(&_fw, _entity_id%d)\n", i)
				}
				for a, i in f.arg_names {
				...
				fmt.sbprintf(
					b,
					"\t\tksim.lane_fact(l, %s, knet.writer_bytes(&_fw), EVENT_%s)\n",
					anchor_ref,
					strings.to_upper(f.name),
				)
				w(b, "\t}\n")
				...
				non_anchor_refs := len(f.entity_names)
				if f.anchor_index >= 0 {non_anchor_refs -= 1}
				if non_anchor_refs > 0 || len(f.arg_names) > 0 {
					w(b, "\tr := knet.reader_make(args)\n")
					for _, i in f.entity_names {
						if i == f.anchor_index {continue}
						fmt.sbprintf(b, "\t_entity_id%d := knet.read_net_id(&r)\n", i)
				...
				for _, i in f.entity_names {
					...
					fmt.sbprintf(
						b,
						"\t_entity%d, _entity_ok%d := ksim.lane_entity(lane, _entity_id%d)\n",
						i,
						i,
						i,
					)
					fmt.sbprintf(
						b,
						"\tif !_entity_ok%d {{return}} // the referenced entity despawned before presentation\n",
						i,
					)
				}
				if f.anchor_index < 0 {
				...
				fmt.sbprintf(b, "\t%s(cast(^%s)ksim.lane_game(lane)", f.fx_proc, cls)
				for entity_type, i in f.entity_types {
					if i == f.anchor_index {
						fmt.sbprintf(b, ", cast(^%s)entity", entity_type)
					} else {
						fmt.sbprintf(b, ", cast(^%s)_entity%d", entity_type, i)
					}
				}
				w(b, ", mine")
			...
			for f in s.facts {
				fmt.sbprintf(
					b,
					"\t{{id = EVENT_%s, audience = %s, fx = _%s_fact_%s}},\n",
					strings.to_upper(f.name),
	...
	if has_succ {
		w(
			b,
			"// ---- host migration (the `_backup`/`_took_over`/`_wiped`/`_migrating` halves) ----\n\n",
		)
		if s.succ_backup != "" {
			fmt.sbprintf(
				b,
		...
		fmt.sbprintf(
			b,
			"// The migration hook table — the generated net facade installs it. A custom\n" +
			"// shell passes it to kboot.boot_migration(&self.%s, self, %s_succ_hooks)\n" +
			"// in ready, after boot_attach. Arms off the SESSION's backup\n" +
	...
	if len(s.entities) > 0 {
		...
		for e in s.entities {
			fmt.sbprintf(
				b,
				"%s_TYPE :: ksess.Entity_Type(%d)\n",
				strings.to_upper(to_snake(e.target)),
				e.type_id,
			)
		}
		w(b, "\n")
		...
		fmt.sbprintf(
			b,
			"// The factory table — %s_entities(self, &self.boot) installs it (below); the raw\n",
			snake,
		)
		...
		fmt.sbprintf(
			b,
			"// Install the generated entity factory on the boot. The generated net facade calls\n" +
			"// this; a custom shell calls it in ready(), after boot_attach. The kind table plus\n" +
			"// the class's event dispatcher means the authority's own\n" +
			...
			cls,
			snake,
			has_events ? fmt.tprintf("_%s_events_thunk", snake) : "nil",
		)
		...
		for e in s.entities {
			tsnake := to_snake(e.target)
			if e.gen_ref || e.gen_of || e.gen_owned || e.gen_mine || e.gen_all || e.gen_despawn || e.gen_teleport {
				fmt.sbprintf(
					b,
			...
			if e.gen_ref {
				fmt.sbprintf(
					b,
					"%s_ref :: proc(id: knet.Net_Id) -> knet.Net_Ref(%s) {{\n\treturn {{id = id}}\n}}\n\n",
					tsnake,
					e.target,
				)
			}
			if e.gen_of {
				fmt.sbprintf(
					b,
					"@(private = \"file\")\n_%s_of_id :: proc(b: ^kboot.Boot, id: knet.Net_Id) -> (^%s, bool) {{\n\te, ok := kboot.boot_entity(b, id, %s_TYPE)\n\treturn cast(^%s)e, ok\n}}\n\n" +
					"@(private = \"file\")\n_%s_of_ref :: proc(b: ^kboot.Boot, ref: knet.Net_Ref(%s)) -> (^%s, bool) {{\n\te, ok := kboot.boot_entity(b, ref.id, %s_TYPE)\n\treturn cast(^%s)e, ok\n}}\n\n" +
					"%s_of :: proc {{_%s_of_id, _%s_of_ref}}\n\n",
					tsnake,
					e.target,
					strings.to_upper(tsnake),
					e.target,
			...
			if e.gen_all {
				fmt.sbprintf(
					b,
					"%s_all :: proc(b: ^kboot.Boot, allocator := context.temp_allocator) -> []knet.Net_Entity(%s) {{\n\treturn kboot.boot_entity_rows(b, %s_TYPE, %s, allocator)\n}}\n\n",
					tsnake,
					e.target,
					strings.to_upper(tsnake),
					e.target,
	...
	// ---- generated game-network facade ----------------------------------
	// A conventional shell has one direct Boot, Session, and Comms field; a
	...
	if s.net_attach || s.net_pump || s.net_frame || s.net_detach {
		...
		has_net_lane :=
			...
			s.step.proc_name != "" ||
			(s.step_auth.proc_name != "" && !s.step_boot)
		all_net_inputs_validated := true
		for ic in s.input_classes {
			if ic.validate == "" && len(ic.constraints) == 0 {
				all_net_inputs_validated = false
				break
			}
		}
		has_net_events := len(s.event_halves) > 0 || has_succ || s.embodied != ""
		for e in s.entities {
			if e.born != "" {has_net_events = true}
		}
		if s.net_attach {
			fmt.sbprintf(
				b,
				"// Attach the complete generated stack. `net_cfg` is a named, validated\n" +
				"// network profile plus any explicit field overrides; zero is Friends_Coop.\n" +
				"// Call once from ready.\n" +
				"%s :: proc(self: ^%s, opts: kboot.Options, net_cfg := kboot.Network_Config{{}}) {{\n",
				net_attach_name,
				cls,
			)
			fmt.sbprintf(
				b,
				"\tassert(self.%s.ses == nil, \"%s called on a live stack — call %s first\")\n",
				s.boot_field,
				net_attach_name,
				net_detach_name,
			)
			if has_net_lane {
				fmt.sbprintf(
					b,
					"\tcfg := kboot.network_configure(&self.%s, net_cfg, kboot.Network_Capabilities{{has_lane = true, all_inputs_validated = %s}})\n",
					s.session_field,
					all_net_inputs_validated ? "true" : "false",
				)
			} else {
				fmt.sbprintf(
					b,
					"\t_ = kboot.network_configure(&self.%s, net_cfg)\n",
					s.session_field,
				)
			}
			fmt.sbprintf(
				b,
				"\tkboot.boot_attach(&self.%s, cast(gd.Node)self.owner, &self.%s, &self.%s, opts)\n",
				s.boot_field,
				s.session_field,
				s.comms_field,
			)
			if len(s.entities) > 0 {
				fmt.sbprintf(b, "\t%s_entities(self, &self.%s)\n", snake, s.boot_field)
			}
			if len(s.messages) > 0 {
			...
			if has_net_lane {
				fmt.sbprintf(
					b,
					"\t%s_lane_init(self, &self.%s, &self.%s, cfg = cfg.lane)\n" +
					"\tkboot.boot_lane(&self.%s, &self.%s)\n",
					snake,
					s.lane_field,
					s.session_field,
					s.boot_field,
			...
			if has_succ {
				fmt.sbprintf(
					b,
					"\tkboot.boot_migration(&self.%s, self, %s_succ_hooks)\n",
					s.boot_field,
					snake,
				)
			}
		...
		if s.net_pump {
			fmt.sbprintf(
				b,
				"// Pump transport/session/lane and forward every declared session event.\n" +
				"// The returned markers and ticks are the game-shaped work left this frame.\n" +
				"%s :: proc(self: ^%s, delta: f64, now: f64) -> kboot.Net_Frame {{\n" +
				"\tassert(self.%s.ses != nil, \"%s comes after %s\")\n" +
				"\tevents, marks, ticks := kboot.boot_pump(&self.%s, delta, now)\n",
				net_pump_name,
				cls,
				s.boot_field,
			...
			if has_net_events {
				fmt.sbprintf(b, "\t%s_events(self, events)\n", snake)
			} else {
				w(b, "\t_ = events\n")
			}
		...
		if s.net_detach {
			fmt.sbprintf(
				b,
				"// Tear down every network layer in dependency order; idempotent.\n" +
				"%s :: proc(self: ^%s) {{\n" +
				"\tkboot.boot_net_detach(&self.%s)\n" +
				"}}\n\n",
				net_detach_name,
				cls,
				s.boot_field,
	...
	if len(s.probes) > 0 {
		...
		for p in s.probes {
			...
			case .Count:
				fmt.sbprintf(
					b,
					"@(private = \"file\")\n_%s_%s :: proc(self: ^%s) -> gd.Int {{\n\treturn gd.Int(len(kboot.boot_entity_rows(&self.%s, %s_TYPE, %s)))\n}}\n\n",
					snake,
					p.name,
					cls,
					s.boot_field,

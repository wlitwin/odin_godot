...
collect_compile_calls :: proc(
	...
	// Collect the root directory plus every nested directory. `filepath.walk`
	// used to include the root itself; the `os.walker_*` API only yields the
	// directory's contents, so we seed the list with `root` explicitly.
	dirs: [dynamic]string
	append(&dirs, root)
	...
	for info in os.walker_walk(&w) {
		if path, werr := os.walker_error(&w); werr != nil {
			warn(nil, "%q error: %v", path, werr)
			continue
		}
		...
		append(&dirs, strings.clone(info.fullpath))
	}
	...
	for dir in dirs {
		...
		if !ok {
			// Not every directory is an Odin package; skip quietly.
			continue
		}
...
transpile_calls :: proc(temple_path: string, calls: []Compile_Call, package_name: string) {
	compiled_path, _ := filepath.join({temple_path, "templates.odin"}, context.allocator)
	handle, errno := os.open(compiled_path, {.Read, .Write, .Create, .Trunc}, os.Permissions_Default_File)
	if errno != nil {
...
write_transpiled_call :: proc(w: io.Writer, call: Compile_Call) -> (ok: bool) {
	...
	case Call_Path:
		...
		file = t.fullpath
		d, rerr := os.read_entire_file(t.fullpath, context.allocator)
		if rerr != nil {
...
embed_parser :: proc(node: ^Node_Embed, parent_path_: rawptr) -> (Template, bool) {
	...
	fullpath, _ := filepath.join({filepath.dir(parent_path), relpath}, context.allocator)

...
Property :: struct {
    ...
    getter: string,
    setter: Maybe(string),
    // For "indexed" properties several names share one getter/setter that takes
    // a leading index argument (e.g. `material_override` / `set_list_stream`).
    index:  Maybe(i64),
}
...
_graph_resolve_type :: proc(graph: ^Graph, type_specifier: Type_Specifier) -> Any_Type {
    ...
    if !is_godot_name {
        ...
        if colon_idx > -1 {
			...
			// Godot 4.5+ typed dictionaries: "typeddictionary::KeyType;ValueType".
			case "typeddictionary":
				{
					semicolon_idx := strings.index_rune(suffix, ';')
					if semicolon_idx < 0 {
						panic(
							fmt.tprintfln("Malformed typeddictionary type string: '%v'", type_string),
						)
					}

					key_string := suffix[:semicolon_idx]
					value_string := suffix[semicolon_idx + 1:]

					// Hint suffixes (after a ':') can appear on either side; drop them,
					// mirroring how typedarray discards its element hint.
					if k := strings.index_rune(key_string, ':'); k > -1 {
						key_string = key_string[:k]
					}
					if v := strings.index_rune(value_string, ':'); v > -1 {
						value_string = value_string[:v]
					}

					typed_dict := new(Typed_Dictionary)
					typed_dict.key_type = _graph_resolve_type(graph, key_string)
					typed_dict.value_type = _graph_resolve_type(graph, value_string)
					return typed_dict
				}
            case:
    ...
    // NOTE: `Root_Type` is a `#no_nil` union, so indexing `graph.types` with a
    // missing key yields a non-nil tag wrapping a nil pointer (its first
    // variant). That silently produces a nil `^Builtin_Class` that later
    // segfaults during rendering. Explicitly check membership and fall back to
    // an opaque `rawptr` for C typedefs the API doesn't describe (e.g.
    // `GDExtensionInitializationFunction`).
    resolved, exists := graph.types[godot_name]
    if !exists {
        fmt.eprintfln("WARN: unresolved type %q, mapping to opaque rawptr", cast(string)godot_name)
        return graph.types["void*"].(^Primitive)
    }

    switch type in resolved {
    case ^Builtin_Class:
        return type
    case ^Engine_Class:
        return type
...
// Resolve a type that is marshalled through Godot's `ptrcall` ABI (engine &
// builtin method args/returns, operators, constructors, utility functions).
//
// In ptrcall, the scripting `float` type is ALWAYS encoded as a 64-bit double:
// `core/variant/method_ptrcall.h` defines `MAKE_PTRARGCONV(float, double)`, so a
// scalar `float` argument is read via `*(const double *)ptr` and a `float`
// return is written via `*(double *)ptr` — 8 bytes either way, regardless of the
// C++ `meta` width ("float" vs "double"). Resolving such a value to a 4-byte
// `f32` (as `_graph_resolve_type` does) makes the binding read the low 4 bytes of
// a double: e.g. a returned `1.0` (0x3FF0000000000000) reads back as `0.0f`, and
// a passed `f32` is interpreted as garbage by the engine.
//
// This only applies to PTRCALL marshalling. Native-struct fields and builtin
// member layouts are real C `float`s (32-bit) and must keep `f32` via
// `_graph_resolve_type`. Vector/real_t types are not the scalar `float` type and
// pass through unchanged.
@(private = "file")
_graph_resolve_ptrcall_type :: proc(graph: ^Graph, type_specifier: Type_Specifier) -> Any_Type {
    resolved := _graph_resolve_type(graph, type_specifier)
    if prim, ok := resolved.(^Primitive); ok && prim != nil && prim.name == "float" {
        return _graph_resolve_type(graph, cast(string)"double")
    }
    return resolved
}
...
_graph_constructor :: proc(graph: ^Graph, api_constructor: ApiClassConstructor) -> Constructor {
    ...
    for api_arg, arg_idx in api_constructor.arguments {
        ...
        constructor.args[arg_idx] = Constructor_Arg {
            name = api_arg.name,
            type = _graph_resolve_ptrcall_type(graph, api_arg.type),
        }
    }
...
_graph_engine_method :: proc(graph: ^Graph, api_method: ApiClassMethod) -> Method {
    ...
    if return_value, ok := api_method.return_value.(ApiClassMethodReturnValue); ok {
        // `meta` is preferred only when it names a concrete type (e.g. the numeric
        // refinements "int32"/"double"/...). As of Godot 4.6 object return values
        // carry non-type metas such as "required" (a nullability hint) which must
        // NOT be treated as the type, otherwise resolution yields a nil type.
        return_type_string := return_value.type
        if meta, has_meta := return_value.meta.(string); has_meta {
            if _, is_type := graph.types[cast(names.Godot_Name)meta]; is_type {
                return_type_string = meta
            }
        }
        method.return_type = _graph_resolve_ptrcall_type(graph, return_type_string)
    }
...
_graph_operator :: proc(graph: ^Graph, api_operator: ApiClassOperator) -> Operator {
    ...
    operator := Operator {
        name        = operator_name,
        return_type = _graph_resolve_ptrcall_type(graph, api_operator.return_type),
        right_type  = nil,
    }

    if right_type_string, ok := api_operator.right_type.(string); ok {
        operator.right_type = _graph_resolve_ptrcall_type(graph, right_type_string)
    }
...
_graph_util_proc :: proc(graph: ^Graph, api_util_func: ApiUtilityFunction) -> Util_Proc {
    ...
    if return_type, has_return_type := api_util_func.return_type.(string); has_return_type {
        util_proc.return_type = _graph_resolve_ptrcall_type(graph, return_type)
    }

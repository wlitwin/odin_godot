...
variant :: proc(class: ^g.Builtin_Class, allocator: mem.Allocator) -> (variant: Variant, render: bool) {
    ...
    for class_enum, enum_idx in class.enums {
        ...
        variant_enum.backing = enum_backing(variant_enum.values, false)
    ...
    if Render_Flags.Methods in render_flags {
        ...
        for class_method, method_idx in class.methods {
            method := Method {
                // BARE engine method name (e.g. "push_back"). The template prefixes
                // the proc identifier with `{{ this.snake_name }}_` itself, and the
                // same `name` is interned for `variant_get_ptr_builtin_method`, which
                // requires the original Godot method name (NOT the Odin proc name).

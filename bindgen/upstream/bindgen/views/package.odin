...
_any_to_odin_name :: proc(type: g.Any_Type, location := #caller_location) -> names.Odin_Name {
    ...
    case ^g.Typed_Dictionary:
        name := cast(names.Odin_Name)fmt.aprintf(
            "!!DEBUG Typed_Dictionary(%v, %v)",
            _any_to_odin_name(as_type.key_type),
            _any_to_odin_name(as_type.value_type),
        )
        fmt.eprintln("WARN! _any_to_odin_name called on Typed_Dictionary: ", name, location)
        return name
    case ^g.Pointer:
        name := cast(names.Odin_Name)fmt.aprintf(
            "!!DEBUG Pointer(%v, %v)",
...
ensure_imports :: proc(imports: ^map[string]Import, type: g.Any_Type, current_package: string) {
    ...
    if typed_dict, is_typed_dict := type.(^g.Typed_Dictionary); is_typed_dict {
        ensure_imports(imports, typed_dict.key_type, current_package)
        ensure_imports(imports, typed_dict.value_type, current_package)
        return
    }
...
resolve_qualified_type :: proc(type: g.Any_Type, current_package: string) -> string {
    ...
    if typed_dict, is_typed_dict := type.(^g.Typed_Dictionary); is_typed_dict {
        key_type := resolve_qualified_type(typed_dict.key_type, current_package)
        value_type := resolve_qualified_type(typed_dict.value_type, current_package)
        return fmt.aprintf("Typed_Dictionary(%v, %v)", key_type, value_type)
    }

...
load_api :: proc(options: Options) -> (api: ^g.Api, ok: bool) {
    data, derr := os.read_entire_file(options.api_file, context.allocator)
    if derr != nil {
        return
    }
...
main :: proc() {
    // Populate the compiled-template procs (core_template, variant_template, ...).
    // Without this their `with` proc pointers are nil and codegen segfaults.
    init_templates()

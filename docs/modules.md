...
- **The swapped module:** live instances keep their state (same-layout fast path, exported
  values preserved by name otherwise — the normal [reload rules](workflow.md#live-editing-show-on-save)),
  and its `reload` hook fires on each instance with the new code. But the module's
  **package globals RESET** — it is a fresh dll, its globals reinitialize.
...
- A module whose dll didn't get built **fails the export with an error naming it** — an
...
| `build_scripts: skipping module 'X' (no .odin sources)` | An empty `modules/X/` dir — add a script or delete the dir. |
| A save's compile error in the editor Output | Same surfacing as the main module — fix the error; the module's old dll stays live meanwhile. |

...
A page for contributors and the curious. It records the house grammar every
package shares — so a new package feels familiar before you read its page — and
the capabilities the kit leaves out, with where to reach instead.
...
Documented as recipes in [session](session.md), not code: character-portable
saves, private per-player state, and player-to-player trade (the mediating-entity
transaction). See the design notes in the repository's knowledge base.
...
Out of scope, and honestly a different toolkit. Everything here assumes a LIVE
session — a shared clock, interpolation timelines, presence, a host holding
...
Not in the kit, and the seam is clean. The kit persists a RUN — [ksave](save.md)
writes POD blobs to a local file, backups ride the wire, host-takeover resumes
from them — but it does not persist a PLAYER across runs against a server store.

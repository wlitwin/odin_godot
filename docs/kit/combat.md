...
on them: deterministic, allocation-free, and engine-free. They run identically inside
predicted commands and host ticks, since client and host execute the same gates from the
same bytes. Projectiles are **not entities**: the shooter's cast is predicted (cost and
cooldown bite instantly), everyone draws local visuals from the same sim math, and only
the *host's* sim deals damage. Visuals are peer-owned; hits are host-validated.
...
- **`hit`/`hurt`** when hp lives somewhere a block can't reach: a transient, an off-entity pool.
...
it is a no-op, so there is no role branch (`spelunker_throw` in `spelunker.odin`).
...
replay, no dedupe, no shooter spoof, no reclaim hole. Use `fire_announce_to` for the tail of a
...
session's own clock, `ksess.session_tick_no(s)`, rather than accumulate a counter, because a
...
Call `php_note_hit` at visual contact and `php_display` wherever hp renders (`rocks.odin`,
`rock_impact`). When the authoritative drop lands, the overlay is consumed, so there is no
double dip. If the host saw a miss, the overlay expires and the bar heals back: briefly wrong,
...
before stepping, remove on hit or expiry, and don't corrupt the index while removing. `pool_tick`
...
- **Cooldowns and effect clocks count NET TICKS**, not seconds, so they are rate-agnostic. Only the
  authority decays them; clients see the countdown through replication.
- A cast raced against the last ticks of a cooldown may mispredict locally, but the command
  still ships, and the host's answer stands.
- `Projectile.vel` is **per tick**. Hit-test with the position *before* `projectile_step`,
  and test the segment even when it returns false, since that tick still happened.

...
backup, and everyone else auto-rejoins with their tokens** — hands-free,
mid-run. The test suite proves it by `kill -9`-ing the host process.
...
4. **Then build yours**, one brick per session, acceptance test green before
   the next brick.

None of this asked you to be a network programmer. You wrote structs with
tagged fields, verbs that return bool, and presentation procs —
single-player-shaped code under a discipline. What that discipline buys is
reconnects, drop-in joins, saves, host migration, and honest-feeling
latency, and you keep the editor, the scene tree, and the engine you already
know.

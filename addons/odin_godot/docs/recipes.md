...
Godot's composition articles all teach the same move: build an entity out of small,
focused **components** — a `HealthComponent`, a `HitboxComponent`, a `VelocityComponent` —
each owning one concern, and attach them to a node. It's a great pattern, and it maps
cleanly onto a single-player game because a single-player game has exactly one runtime (the
frame) and exactly one authority (you). Neither of those holds in a `kit` game, so the
component pattern doesn't transfer whole — and that's why building with `kit` + `play` can
feel like it's missing a recipe.

It isn't. There *is* a compositional pattern here. It's just decomposed along a different
axis.

> **Single-player composition asks _"what are its parts?"_ — health, movement, hitbox.
> kit + play composition asks _"where does each part live across the wire?"_ Same behavior,
> sliced by authority instead of by concern.**

This page is that recipe: the two layers of composition, the seven slots a networked
gameplay item is sliced into, one fully worked example (a gun), and the same skeleton shown
five different ways. It assumes you're comfortable with the toolkit's three foundations —
**state-not-messages** (peers converge on replicated state, they don't send each other
events), **verbs-not-RPCs** (a client's only reach into the world is a command that runs on
the host), and **the two timelines** (the fixed net tick the host simulates on, distinct
from the render frame). This page is the applied "how" on top of that "why", and it leans on
all three.
...
The direct analog to attaching components is real, and it's the first layer. **An entity is
a struct, and its behaviors are `play` primitives embedded as fields.** From the co-op
twin-stick dogfood:
...
`Runner` *has-a* gun state machine, *has-a* damage edge, *has-a* revive channel. Adding a
behavior is adding a field — exactly the ergonomics of attaching a component node. The thing
that makes it work is **scriptgen recursing into nested structs**: embedding
`play.Machine(Gun_State)` composes its inner `cur: Gun_State \`gd:"replicate"\`` into the
`Runner`'s own wire descriptor automatically, as if you'd written a flat field. The
machine's local `shadow` is untagged, so it never crosses the wire. You embed one field and
get both the replicated state and its local presentation shadow.

The differences from Godot components are worth stating plainly:
...
That's the whole of Layer 1: **compose an entity by embedding `play` primitives; scriptgen
carries their wire, export, `@onready`, and signal needs up into the parent for free.**

And not just their state — **their verbs too.** A block that declares a `@(gd_command)` on
itself has that command hoisted onto whatever entity embeds it (with the wielder threaded in),
so a self-contained gameplay block — a gun that carries its own `fire`, a door its own `open` —
drops in as one field and brings both its replicated state *and* its Intent seam. That's what
makes the composition recursive: you keep leveling up (gun → loadout → character) without
...
The primitives you compose from. Four are pure (safe in any timeline); one touches the
engine and is presentation-only. Each usually fills a particular *slot* in Layer 2 (next
section) — the last column previews that.
...
scratch* — they live in an entity's host brain or its render-side `view`, off the
replication boundary by construction. That's the discipline that keeps `play` opt-in and
un-magical: it never smuggles anything onto the network.
...
**a single behavior — "the gun" — isn't one component. It's one concern smeared across the
...
**Not every item fills every slot** — that's the point, and the [matrix](#the-same-skeleton-five-ways)
below shows it. A cosmetic pickup is just State + Authority + Cue with no Intent. A mob has
no player behind it, so it skips Intent, Prediction, and Reconcile entirely. The skeleton is
a *checklist to walk*, not a mandate to fill.

Two of these slots — **Prediction** and **Reconcile** — are the ones single-player never
forces you to think about, and they're where the feel of a `kit` game is won or lost. The
rest of this page is mostly about getting them right.

---

## Worked example: the fire / reload / jam gun

The gun is the flagship because it fills **all seven** slots and composes the whole toolbox.
Its states are `Ready → (fire) → Reloading` on an empty mag, or `Ready → Jammed` on a dud
...
**② Cadence** — one clock gates *every* state (a fire cooldown, a reload gap, a jam-clear
...
**③ Intent** — the one seam. A `predict` command runs on the *client's* predictor **and** the
...
**④ Authority** — the host owns truth. The command hook decides fire-vs-jam-vs-clear, and
...
**⑤ Prediction** — the client runs the *identical* verdict locally, so the shot flies this
frame with zero felt lag. This only works because the jam is **deterministic from state both
sides share** — the same seed produces the same roll on both machines, so the client isn't
guessing, it's computing the answer the host will reach:
...
The client predicts the fire, the ammo decrement, the auto-reload gap (it just holds the
pacer for the reload duration), and the jam — all locally, all instant.

**⑥ Reconcile** — prediction and truth must re-align without ever clawing back a *good*
in-flight guess. The trick: snap only on a host **transition**, because transitions land at
mag boundaries where prediction and authority already agree:
...
**⑦ Cue** — presentation reacts once per change, on every screen, driven by `play.step` off
...
The host observes its own `set`; each client observes replication deliver `cur`; **both**
run `step`, so the enter/exit cues fire identically on every screen with no RPC and no
coordination. That symmetry — host and client running the same `step` off the same state —
is the engine underneath the whole recipe.

---

## The same skeleton, five ways

The gun fills all seven slots; most items don't. Here are five real items from the dogfood
...
pulse before granting `hp`. It skips Prediction/Reconcile because you can't honestly predict
"did my friend agree I'm in reach" — but the *channel bar* is pure local presentation off the
replicated `rev_pct`, built once and driven with `play.marker`:
...
behavior is one host proc, its timers a `play.Deadlines`-style bundle of `Pace` on the net
tick:
...
There's no per-player Intent, so no Prediction/Reconcile — but it's the *same* Machine as the
gun's, just owned by a singleton instead of a runner:
...
credits the stat, and despawns the entity. Pure Authority + State + Cue. The client "feels"
it as its scrap counter ticking up — the least slots an item can fill and still be a thing:
...
If you can answer all seven for your item — and cross off the ones it legitimately skips —
you've designed a networked gameplay item that will feel right, without re-deriving the
client/host dance each time.

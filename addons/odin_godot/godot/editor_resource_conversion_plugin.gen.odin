...
editor_resource_conversion_plugin__converts_to :: proc "contextless" (
    ...
    if __ptr == nil {
        _gde_name := new_string_name_cstring("_converts_to", true)
...
editor_resource_conversion_plugin__convert :: proc "contextless" (
    ...
    @(static) __ptr: __bindgen_gde.MethodBindPtr
    if __ptr == nil {
        _gde_name := new_string_name_cstring("_convert", true)
        __ptr = __bindgen_gde.classdb_get_method_bind(&__class_name, &_gde_name, 325183270)
    }
    self := self
    resource_ := resource_
    args := []__bindgen_gde.TypePtr {

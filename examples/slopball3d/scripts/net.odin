...
// Transport + identity, slopball's shape: the four wire forwards, the
// Host/Join doors, and who this peer IS (SLOP3_NAME / SLOP3_TOKEN env override
...
port :: proc() -> int {
	return gd.env_int("SLOP3_PORT", DEFAULT_PORT)
}

my_name :: proc() -> string {
	n := gd.env_string("SLOP3_NAME", "")
	return n == "" ? "kicker" : n
}

my_token :: proc() -> u64 {
	return ksave.token({env = "SLOP3_TOKEN", path = "user://slop3_token"})
}

// ---- the four transport forwards (netgd.Session_Wire owns what's behind them) ----

@(gd_method)
slopball3_on_packet :: proc(self: ^Slopball3, id: gd.Int, packet: gd.Packed_Byte_Array) {
	netgd.wire_receive(&self.boot.wire, id, packet)
}

@(gd_method)
slopball3_on_peer_left :: proc(self: ^Slopball3, id: gd.Int) {
	ksess.session_peer_disconnected(&self.ses, ksess.Peer_Id(id))
}

@(gd_method)
slopball3_on_net_up :: proc(self: ^Slopball3) {
	ksess.session_client_join(&self.ses)
}

@(gd_method)
slopball3_on_net_down :: proc(self: ^Slopball3) {

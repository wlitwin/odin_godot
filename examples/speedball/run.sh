#!/usr/bin/env bash
# ----------------------------------------------------------------------------
# SOLO gate for speedball. Proves the contested-object loop runs headless as
...
# bytes. Prints SPEEDBALL_SINGLE_OK.
#
#   nix develop --command bash -c 'bash examples/speedball/run.sh'
# ----------------------------------------------------------------------------
set -euo pipefail
...
if grep -qE "signal 11|must be overridden|Required virtual" "$ELOG"; then
    echo "SPEEDBALL_SINGLE_FAIL: editor-open smoke logged a crash / missing virtual"
    tail -n 20 "$ELOG"; exit 1
fi
echo "speedball: editor-open smoke clean"

SLOG="$(mktemp)"; trap 'rm -f "$ELOG" "$SLOG"' EXIT
set +e
SPB_ROLE=single SPB_BOT=striker SPB_GOALS=1 SPB_TOKEN=901 \
    "$GODOT" --headless --path "$PROJ" --quit-after 3000 >"$SLOG" 2>&1
set -e

ok=1
for want in "SPB_HOSTING" "SPB_WORLD_UP" "SPB_STARTED" "SPB_KICK" "SPB_GOAL team=1" "SPB_MATCH winner=1" "SPEEDBALL_DONE"; do
    grep -q "$want" "$SLOG" || { echo "missing: $want"; ok=0; }
done
if grep -qE "SCRIPT ERROR|signal 11|ODIN_SCRIPT_PANIC" "$SLOG"; then
    echo "speedball: runtime errors in the solo log"; ok=0
fi

if ((ok==1)); then
    echo "SPEEDBALL_SINGLE_OK proved: headless contested-ball loop — dribble, kick, predicted goal reset, delta-lane score, solo"
    exit 0
fi
tail -n 40 "$SLOG"
echo "SPEEDBALL_SINGLE_FAIL"
exit 1

#!/usr/bin/env bash
# ----------------------------------------------------------------------------
# CO-OP WEB (WebRTC) — TWO real headless-Chrome peers run the SAME exported arena.tscn over a
# genuine browser-native WebRTC data channel (brokered by the WebSocket signaling server) and
# prove the PEER-AUTHORITATIVE arena guarantees IN-BROWSER: both owner-auth pawns synced, the
# firer's OWNER-LOCAL bullet immediacy (BULLET_LOCAL on the client + BULLET_REMOTE on the host),
# and a peer-authoritative kill agreed by both peers. SAME unified codebase + replication nodes
# as the native ENet test, now over WebRTC.
#
# Prints ARENA_WEB_OK on a verified two-browser run, or ARENA_WEB_BUNDLED when the build+export
# succeeded but the browser step was skipped (no node/Chrome/puppeteer) — gated like the other
# browser tests.   nix develop --command bash -c 'bash examples/coop_arena/coop_web_run.sh'
# ----------------------------------------------------------------------------
set -uo pipefail
...
if [[ ! -f "$PROJ/out/libodin_godot.wasm" || ! -f "$PROJ/out/index.side.wasm" ]]; then
    echo "ARENA_WEB_FAIL: export did not bundle the SIDE_MODULE + dlink runtime"
    exit 1
fi
...
if ! command -v node >/dev/null 2>&1 || [[ ! -x "$CHROME" ]]; then
    echo "ARENA_WEB_BUNDLED: browser step skipped (no node and/or Chrome at \$CHROME)."
    echo "  Manual: node tests/webrtc/signal_server.mjs 9080  +  bash tests/web/serve.sh examples/coop_arena/out $PORT"
...
if [[ ! -d "$PROJ/node_modules/puppeteer-core" || ! -d "$PROJ/node_modules/ws" ]]; then
    if [[ -d "$ROOT/tests/web/node_modules/puppeteer-core" && -d "$ROOT/tests/web/node_modules/ws" ]]; then
        ...
        echo "ARENA_WEB_BUNDLED: browser step skipped (could not obtain puppeteer-core + ws)."
        exit 0
    fi
fi
...
rc=1
for try in 1 2; do
    echo "==> two-browser arena attempt $try (signaling ws://127.0.0.1:$SIGPORT)"
